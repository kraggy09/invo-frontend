...
const BillingTabs = () => {
  ...
  const menuItems: MenuProps["items"] = isRetailOnly
    ? [
        {
          key: "RETAIL",
          label: "Retail Bill",
    ...
    <div className="flex flex-col h-full bg-white relative">
        ...
        {tabs.length === 0 ? (
          ...
          tabs.map((tab) => (
            <div
              ...
              className={`group relative p-4 rounded-2xl border-2 transition-all cursor-pointer ${tab.fiveMinutes ? "stale-blink" : ""} ${
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {(() => {
                  const bill = bills.find(
                    (b) => b.id.toString() === tab.key.toString(),
                  );
                  const hasItems =
                    bill && bill.purchased && bill.purchased.length > 0;
                  const deleteButton = (
                    <Button
                      type="text"
                      size="middle"
                      ...
                      className={`opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center rounded-xl w-9 h-9 hover:bg-red-50 hover:text-red-600 ${
                        currentBillingId.toString() === tab.key
                          ? "text-indigo-600"
                          : "text-gray-400"
                      }`}
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span
                  className={`font-black text-sm tracking-tight ${currentBillingId.toString() === tab.key ? "text-indigo-900" : "text-gray-600"}`}
                >
                  Session {tab.label}
                </span>
                {bills.find((b) => b.id === tab.key)?.billType && (
                  <span
                    className={`text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase ml-auto ${currentBillingId.toString() === tab.key ? "bg-indigo-50 text-indigo-600 border border-indigo-100" : "bg-gray-50 text-gray-400 border border-transparent"}`}
                  >
                    {bills.find((b) => b.id === tab.key)?.billType ===
                    "SUPERWHOLESALE"
                      ? "SW"
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center justify-between px-2">
          ...
          <span className="text-[10px] font-black text-indigo-500 uppercase font-bold">
            Synchronized
          </span>
        </div>
      </div>
