      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-[500px]">
        ...
        >
          <header className="flex items-center flex-col justify-center">
            ...
            <p className="text-sm font-black mt-1 uppercase tracking-widest border border-black px-4 py-0.5 rounded-md">
              Return Receipt
            </p>
              ...
              <span id="left" className="mr-10 text-left">
                ...
                <p>
                  ...
                  {returnBill?.originalBill?.id
                    ...
                    : returnBill?.originalBill?._id?.slice(-8).toUpperCase() ||
                      "N/A"}
                </p>
                <p>Date: {dayjs(returnBill.createdAt).format("DD/MM/YYYY")}</p>
              </span>
              <span id="right" className="text-right">
                <p>Mode: {paymentMode}</p>
                <p>Time: {dayjs(returnBill.createdAt).format("hh:mm A")}</p>
              </span>
            </div>
          </header>
                ...
                {returnBill?.items?.map((item: any, index: number) => {
                  return (
                    <tr key={item._id || index} className="border border-black">
                      ...
                      <td>
                        <p className="text-center py-1">
                          {item.returnTotal % 1 !== 0
                            ? item.returnTotal.toFixed(2)
                            : item.returnTotal}
                        </p>
                      </td>
                    </tr>
